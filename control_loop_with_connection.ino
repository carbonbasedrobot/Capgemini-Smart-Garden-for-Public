...
void loop() {
  ...
  //int tempWaterLevel = readTankHeight(); //variable used to detect changes in tanklevel
  ...
  if (WiFi.status() != WL_CONNECTED) {  //tries to connect to the cloud if not connected
    ...
    if (WiFi.status() == WL_CONNECTED) {  // IF CONNECTION SUCCEEDS, BUT IT WAS DISCONNECTED AT SOME POINT, THIS INITIALIZES AZURE THINGS
      ...
      //if (millis() > telemetryNextSendTimeMs || abs(tankLevel - tempWaterLevel) > 10)  //if its been long enough since the last message was sent or the tank level has changed by 5%
        if(millis() > telemetryNextSendTimeMs)
      {
        // if (abs(zoneArray[0].getTankLevel() - tempWaterLevel) > 10) {
